...
def stft(sig, frameSize, overlapFac=0.5, window=np.hanning):
    ...
    samples = np.append(np.zeros(np.int32(np.floor(frameSize/2.0))), sig)    
    # cols for windowing
    cols = np.int32(np.ceil( (len(samples) - frameSize) / float(hopSize)) + 1)
...
def plotstft(audiopath, binsize=2**10, plotpath=None, colormap="gray", channel=0, name='tmp.png', alpha=1, offset=0):
    ...
    image = image.resize((30, 256))
    image.save(name)


file = open('testing_list.csv', 'r')
